...

  {
    ...
    for (i in 1:N_long) {
      ...
      if(has_treg) {
        ...
        nu[i] = (lambda[lambda_ind[i]]*(theta_g[nn[i]] + xb) + theta_resid[nn[i], lambda_ind[i]])*(alpha[lambda_ind[i], jj[i]] + a_design[nn[i], ]*alpha_r_l);
      } else {
        nu[i] = (lambda[lambda_ind[i]]*(theta_g[nn[i]]) + theta_resid[nn[i], lambda_ind[i]])*(alpha[lambda_ind[i], jj[i]] + a_design[nn[i], ]*alpha_r_l);
      }
