...
//           beta[i, d] = 0;
//           // print("loop iteration beta: ", beta[i, d]);
//         }
//       }
//     }
//     {
...
//   // beta_l ~ normal(0, 5);
//   // y ~ bernoulli_logit((alpha[jj] .* (theta[nn] + (x[nn, ] * beta))) - delta[jj]);
//   // y ~ bernoulli_logit((alpha[jj] .* (theta[nn])) - delta[jj]);
//   {
//     vector[N_long] nu;
//     for (i in 1:N_long) {
//       nu[i] = (( (theta_g[nn[i], ]*col(lambda_g, nn_lindex[i])) + theta[nn[i], ] + (x[nn[i], ] * beta))*col(alpha, jj[i])) - delta[jj[i]]; // theta_g[nn[i], ] + 
//     }
