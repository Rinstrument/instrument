...
alpha = rtruncnorm(j, a = 0, mean = 1, sd = 0.5)
delta = rnorm(j, 0, 1)
lambda = c(1, -2, 4)
#beta = 0.5
...
apply(data[, (n1l+2):ncol(data)], 2, table)
...
validation_lower = c(rep(0, j), rep(-5, j), rep(-1, n1l))
validation_upper = c(rep(8, j), rep(5, j), rep(1, n1l))
...
post_mean = mapply(\(row, ind_max) {mean(x[row, 1:ind_max])}, row = 1:nrow(x), ind_max = indices)
...
View(cbind(delta, post_mean[42:(42 + 41 - 1)]))
...
View(cbind(thetas[,4], post_mean[model_indices[[6]] + 1]))
cor(post_mean,
    true)
# cor(
#   theta,
...
hist(post_mean[model_indices[[4]] + 1])
